      ...
      <aside>
          <div id="sidebar"  class="nav-collapse ">
              ...
              <ul class="sidebar-menu" id="nav-accordion">
                  ...
                  <li class="sub-menu">
                      <a class="active" href="/cgi/shopcar/manage">
                          <i class="fa fa-book"></i>
                          <span>购物车管理</span>
                      </a>
                  </li>
          ...
          <section class="wrapper">
              <html>
                      ...
                      <div class="table-responsive">
                          <table class="table table-hover">
                              ...
                              <c:forEach items="${content}" var="shopcar_goods">
                                  <tr>
                                      <td>${shopcar_goods.shopcar.id}</td>
                                      <td>${shopcar_goods.shopcar.customer.realname}</td>
                      ...
                      <div align="center" class="page1">
                          ...
                          <a href="/cgi/shopcar/manage?pageNow=1" >首页</a>
                          <c:choose>
                              <c:when test="${page.pageNow - 1 > 0}">
                                  <a href="/cgi/shopcar/manage?pageNow=${page.pageNow - 1}">上一页</a>
                              </c:when>
                              <c:when test="${page.pageNow - 1 <= 0}">
                                  <a href="/cgi/shopcar/manage?pageNow=1" >上一页</a>
                              </c:when>
                          </c:choose>
                          <c:choose>
                              <c:when test="${page.totalPageCount==0}">
                                  <a href="/cgi/shopcar/manage?pageNow=${page.pageNow}">下一页</a>
                              </c:when>
                              <c:when test="${page.pageNow + 1 < page.totalPageCount}">
                                  <a href="/cgi/shopcar/manage?pageNow=${page.pageNow + 1}">下一页</a>
                              </c:when>
                              <c:when test="${page.pageNow + 1 >= page.totalPageCount}">
                                  <a href="/cgi/shopcar/manage?pageNow=${page.totalPageCount}">下一页</a>
                              </c:when>
                          </c:choose>
                          <c:choose>
                              <c:when test="${page.totalPageCount==0}">
                                  <a href="/cgi/shopcar/manage?pageNow=${page.pageNow}" >   尾页</a>
                              </c:when>
                              <c:otherwise>
                                  <a href="/cgi/shopcar/manage?pageNow=${page.totalPageCount}">尾页 </a>
                              </c:otherwise>
                          </c:choose>
                      </div>
